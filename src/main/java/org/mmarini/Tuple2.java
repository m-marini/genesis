...
public class Tuple2<T1, T2> {
    ...
    /**
     * Returns the stream of pair element from parameter lists
     *
     * @param t   the first list
     * @param u   the second list
     * @param <T> the type od first items
     * @param <U> the type of second items
     */
    public static <T, U> Stream<Tuple2<T, U>> zip(List<T> t, List<U> u) {
        requireNonNull(t);
        requireNonNull(u);
        return IntStream.range(0, min(t.size(), u.size()))
                .mapToObj(i -> of(t.get(i), u.get(i)));
    }
